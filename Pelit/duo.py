...
paraf = open('duo_para.txt')
				 # 0 = min Kelly, 1 = panos, 2 = min lunastus
para = [float(x) for x in paraf.readline().split(',')]

				# 0 - kuinka monta omaa hepoa systeemissä, 1 - 1. lähdön omat hepot, 2 - 2. lähdön omat hepot
for line in open('duo.txt'):
    systeemi.append([int(x) for x in line.split(',')])

kerroinxml = kertoimet.bs(koodi, lahto, 'duo')
...
out = open('/home/kari/Python/Pelit/duo.peli', 'w')
for yhd in kerroinxml.find_all('probable'):
    y = [int(y) for y in yhd['combination'].split('-')]

    if yhdistelma.ok(y, systeemi):
        kerroin = float(yhd.string.replace(',', '.'))
        if int(kerroin) == 0:
            kerroin = float(jako.replace(',','.'))                  # max kerroin jos yhdistelmää ei pelattu
        omakerr = 10000.0
